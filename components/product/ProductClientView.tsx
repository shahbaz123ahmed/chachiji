  ...
  return (
    <div className="bg-[#FFFFFF] min-h-screen pb-24 sm:pb-28">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-2 text-xs text-[#777777] flex items-center gap-1.5 font-medium">
        <Link href="/" className="hover:text-[#8B3E3E]">Home</Link>
        <span>/</span>
        <Link href={`/shop/${product.category}`} className="hover:text-[#8B3E3E] capitalize">
          {product.category}
        </Link>
        <span>/</span>
        <span className="text-[#8B3E3E] font-bold truncate max-w-xs">{product.name}</span>
      </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14">
          ...
          <div className="lg:col-span-6 space-y-4">
            {/* Big Active Image */}
            <div className="relative aspect-square w-full rounded-3xl overflow-hidden bg-[#FCE9D6] border-2 border-[rgba(51,51,51,0.10)] shadow-md">
              <Image
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-1.5">
                {product.isBestseller && (
                  <span className="bg-[#8B3E3E] text-[#FFFFFF] text-xs uppercase font-bold tracking-wider px-3 py-1 rounded-full shadow-md flex items-center gap-1">
                    <Sparkles className="w-3 h-3 text-[#F7A77A]" />
                    Bestseller
                  </span>
                )}
                {product.discountPercentage > 0 && (
                  <span className="bg-[#F7A77A] text-[#333333] text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full shadow-md">
              ...
              <button
                ...
                className="absolute top-4 right-4 w-10 h-10 rounded-full bg-[#FFFFFF] flex items-center justify-center text-[#333333] hover:text-[#8B3E3E] shadow-sm transition-all border border-[rgba(51,51,51,0.10)]"
                  ...
                  <button
                    ...
                    className={`relative w-20 h-20 rounded-2xl overflow-hidden shrink-0 border-2 transition-all ${
                      activeImageIndex === idx
                        ? "border-[#8B3E3E] scale-105 shadow-md"
          ...
          <div className="lg:col-span-6 space-y-6">
            <div>
              ...
              <div className="flex items-center justify-between gap-4 mb-2">
                <span className="text-xs font-bold uppercase tracking-widest text-[#8B3E3E]">
                ...
                <div className="flex items-center gap-1.5 text-xs text-[#E5A93C]">
              ...
              <h1 className="font-serif text-3xl sm:text-4xl lg:text-5xl font-bold text-[#333333] leading-tight">
                {product.name}
              </h1>
              {product.hindiName && (
            ...
            <div className="flex items-baseline gap-3 p-4 rounded-2xl bg-[#FFFFFF] border-2 border-[rgba(51,51,51,0.10)] shadow-xs">
              <span className="font-serif text-3xl sm:text-4xl font-bold text-[#8B3E3E]">
              ...
              {selectedVariant.mrp > selectedVariant.price && (
                <>
                  ...
                  <span className="bg-[#F7A77A] text-[#333333] text-xs font-bold px-2.5 py-0.5 rounded-full shadow-2xs">
                    Save ₹{savings}
                  </span>
                </>
              )}
            ...
            {product.variants.length > 1 && (
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-[#8B3E3E] mb-2.5">
                    ...
                    <button
                      ...
                      className={`px-4 py-2.5 rounded-xl text-xs font-bold border-2 transition-all ${
                        selectedVariant.id === v.id
                          ? "bg-[#8B3E3E] text-[#FFFFFF] border-[#8B3E3E] shadow-sm"
                          : "bg-white text-[#333333] border-[rgba(51,51,51,0.18)] hover:border-[#8B3E3E]"
                      }`}
                    >
                      <span>{v.weight}</span>
            ...
            <div className="space-y-3 pt-2">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center border border-[rgba(51,51,51,0.15)] rounded-xl bg-white p-1 shadow-2xs">
                  <button
                    ...
                    className="p-2 text-[#333333] hover:bg-[#FCE9D6] rounded-lg transition-colors"
                    aria-label="Decrease quantity"
                  >
                    <Minus className="w-4 h-4" />
                  </button>
                  <span className="w-10 text-center text-sm font-bold text-[#333333]">
                    {quantity}
                  </span>
                  <button
                ...
                {/* Add To Cart: Solid #8B3E3E */}
                <button
                  onClick={() => handleAddToCart(true)}
                  className="flex-1 bg-[#8B3E3E] hover:bg-[#733232] text-[#FFFFFF] font-bold text-xs sm:text-sm py-4 px-6 rounded-xl shadow-md transition-all flex items-center justify-center gap-2 active:scale-95"
                >
              ...
              {/* Buy Now: Solid Vibrant #F7A77A */}
              <button
                onClick={handleBuyNow}
                className="w-full bg-[#F7A77A] hover:bg-[#E89565] text-[#333333] font-bold text-xs sm:text-sm py-4 px-6 rounded-xl shadow-sm hover:shadow-md transition-all flex items-center justify-center gap-2 active:scale-95"
              >
                <Zap className="w-4 h-4 fill-[#333333]" />
                <span>Buy Now • Instant Checkout</span>
              </button>
            </div>

            {/* Pincode Estimator */}
            <div className="bg-[#FCE9D6] p-4 rounded-2xl border border-[rgba(51,51,51,0.10)] space-y-2 shadow-xs">
              <span className="text-xs font-bold uppercase tracking-wider text-[#8B3E3E] flex items-center gap-1.5">
                <MapPin className="w-3.5 h-3.5 text-[#8B3E3E]" />
              ...
              <form onSubmit={handleCheckPincode} className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-[#FFFFFF] border border-[rgba(51,51,51,0.15)] rounded-xl px-3 py-2 text-xs text-[#333333] font-semibold focus:outline-none focus:border-[#8B3E3E]"
                />
                <button
                  type="submit"
                  className="bg-[#8B3E3E] text-[#FFFFFF] text-xs font-bold px-4 py-2 rounded-xl hover:bg-[#733232] transition-colors shadow-xs"
                >
                  Check
                </button>
              </form>
              {pincodeStatus && (
                <p className="text-xs font-bold text-[#8B3E3E]">{pincodeStatus}</p>
              )}
            </div>

            {/* Trust Badges Strip */}
            <div className="grid grid-cols-2 gap-3 pt-2 text-xs text-[#333333]">
              <div className="flex items-center gap-2 bg-[#FFFFFF] p-3 rounded-xl border border-[rgba(51,51,51,0.10)] shadow-2xs">
                <Sun className="w-4 h-4 text-[#8B3E3E] shrink-0" />
                <span className="text-[11px] font-bold">100% Sun Cured in Martabans</span>
              </div>
              <div className="flex items-center gap-2 bg-[#FFFFFF] p-3 rounded-xl border border-[rgba(51,51,51,0.10)] shadow-2xs">
                <Droplets className="w-4 h-4 text-[#8B3E3E] shrink-0" />
                <span className="text-[11px] font-bold">Kachchi Ghani Mustard Oil</span>
              </div>
              <div className="flex items-center gap-2 bg-[#FFFFFF] p-3 rounded-xl border border-[rgba(51,51,51,0.10)] shadow-2xs">
                <ShieldCheck className="w-4 h-4 text-[#8B3E3E] shrink-0" />
                <span className="text-[11px] font-bold">Zero Chemical Preservatives</span>
              </div>
              <div className="flex items-center gap-2 bg-[#FFFFFF] p-3 rounded-xl border border-[rgba(51,51,51,0.10)] shadow-2xs">
                <Truck className="w-4 h-4 text-[#8B3E3E] shrink-0" />
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="bg-[#FCE9D6] text-[#333333] p-8 sm:p-10 rounded-3xl border border-[rgba(51,51,51,0.10)] shadow-sm">
          <span className="text-xs font-bold uppercase tracking-[0.2em] text-[#8B3E3E] block mb-1">
            Craftsmanship Highlights
          </span>
          <h3 className="font-serif text-2xl sm:text-3xl font-bold mb-6 text-[#333333]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {product.whyYouWillLoveIt.map((reason, i) => (
              <div key={i} className="bg-[#FFFFFF] p-4 rounded-2xl border border-[rgba(51,51,51,0.10)] flex items-start gap-3 shadow-xs">
                ...
                <p className="text-xs text-[#333333] font-medium leading-relaxed">{reason}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-[#FFFFFF] rounded-3xl border border-[rgba(51,51,51,0.10)] p-6 sm:p-10 shadow-sm space-y-4">
          ...
          <div className="border-b border-[rgba(51,51,51,0.10)] pb-4">
            <button
              ...
              className="w-full flex items-center justify-between text-left py-2 font-serif text-xl font-bold text-[#333333] hover:text-[#8B3E3E]"
            >
              <span>Heritage &amp; Description</span>
              <ChevronDown
                className={`w-5 h-5 text-[#8B3E3E] transition-transform duration-200 ${
                  openAccordion === "story" ? "rotate-180" : ""
                }`}
              />
            </button>
            {openAccordion === "story" && (
              <div className="pt-3 text-xs sm:text-sm text-[#333333] font-medium leading-relaxed space-y-3">
                <p>{product.description}</p>
                {product.curingProcess && (
                  <p>
                    <strong className="text-[#8B3E3E]">Curing Method:</strong> {product.curingProcess}
                  </p>
                )}
                {product.harvestOrigin && (
                  <p>
                    <strong className="text-[#8B3E3E]">Harvest Origin:</strong> {product.harvestOrigin}
                  </p>
                )}
              </div>
            )}
          ...
          <div className="border-b border-[rgba(51,51,51,0.10)] pb-4">
            ...
            {openAccordion === "ingredients" && (
              <div className="pt-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-xs text-[#333333]">
                  {product.ingredients.map((ing, i) => (
                    <div key={i} className="flex items-center gap-2 bg-[#FCE9D6] p-3 rounded-xl border border-[rgba(51,51,51,0.08)] font-semibold">
            ...
            {openAccordion === "nutrition" && (
              <div className="pt-3">
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-center">
                  <div className="bg-[#FCE9D6] p-3.5 rounded-xl border border-[rgba(51,51,51,0.08)]">
                    <span className="block text-[11px] text-[#555555] font-semibold">Energy</span>
                    <span className="font-serif text-xl font-bold text-[#8B3E3E]">
          ...
          <div className="border-b border-[rgba(51,51,51,0.10)] pb-4">
            ...
            {openAccordion === "pairings" && (
              <div className="pt-3 space-y-2 text-xs sm:text-sm text-[#333333] font-medium">
                <ul className="space-y-2">
                  {product.pairings.map((p, idx) => (
                    <li key={idx} className="flex items-center gap-2">
                      <Sparkles className="w-4 h-4 text-[#F7A77A] shrink-0" />
                      <span>{p}</span>
                    </li>
                  ))}
                </ul>
          ...
          <div>
            ...
            {openAccordion === "storage" && (
              <div className="pt-3 text-xs sm:text-sm text-[#333333] font-medium leading-relaxed">
        ...
        <div className="bg-[#FFFFFF] rounded-3xl border border-[rgba(51,51,51,0.10)] p-6 sm:p-10 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[rgba(51,51,51,0.10)] pb-6 mb-8">
            <div>
              ...
              <h3 className="font-serif text-2xl sm:text-3xl font-bold text-[#333333] mt-1">
                Customer Reviews
              </h3>
              <div className="flex items-center gap-2 mt-2">
                ...
                <span className="font-bold text-sm text-[#333333]">{product.rating} out of 5</span>
            ...
            <button
              ...
              className="inline-flex items-center gap-2 bg-[#8B3E3E] hover:bg-[#733232] text-[#FFFFFF] font-bold text-xs px-5 py-3 rounded-xl transition-colors self-start sm:self-auto shadow-sm"
            >
          ...
          <div className="space-y-4">
            <div className="bg-[#FCE9D6] p-5 rounded-2xl border border-[rgba(51,51,51,0.10)]">
              ...
              <p className="text-xs text-[#333333] font-medium leading-relaxed mb-3">
              ...
              <div className="flex items-center gap-2 text-[11px]">
                <span className="font-bold text-[#8B3E3E]">Rakesh N. (Verified Purchaser)</span>
            ...
            <div className="bg-[#FCE9D6] p-5 rounded-2xl border border-[rgba(51,51,51,0.10)]">
              ...
              <div className="flex items-center gap-2 text-[11px]">
                <span className="font-bold text-[#8B3E3E]">Meera S. (Verified Purchaser)</span>
                <span className="text-[#777777]">•</span>
      ...
      <div className="fixed bottom-0 inset-x-0 z-30 bg-[#FFFFFF] border-t border-[rgba(51,51,51,0.15)] p-3 sm:hidden shadow-2xl flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => handleAddToCart(true)}
            className="bg-[#8B3E3E] text-[#FFFFFF] text-xs font-bold py-3 px-4 rounded-xl flex items-center gap-1.5 shadow-md active:scale-95"
          >
          ...
          <button
            onClick={handleBuyNow}
            className="bg-[#F7A77A] text-[#333333] text-xs font-bold py-3 px-4 rounded-xl flex items-center gap-1 shadow-md active:scale-95"
          >
            <Zap className="w-3.5 h-3.5 fill-[#333333]" />
            <span>Buy Now</span>
          </button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
          <div className="bg-[#FFFFFF] rounded-3xl max-w-md w-full p-6 border border-[rgba(51,51,51,0.15)] shadow-2xl animate-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-[rgba(51,51,51,0.10)] pb-3 mb-4">
              <h4 className="font-serif text-xl font-bold text-[#333333]">
                Review {product.name}
              </h4>
              <button
                onClick={() => setIsReviewModalOpen(false)}
                className="text-[#333333] hover:text-[#8B3E3E]"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {reviewSubmitted ? (
              <div className="py-8 text-center text-[#8B3E3E]">
                <CheckCircle2 className="w-12 h-12 text-[#8B3E3E] mx-auto mb-2" />
              ...
              <form onSubmit={handleReviewSubmit} className="space-y-3.5">
                ...
                <button
                  type="submit"
                  className="w-full bg-[#8B3E3E] hover:bg-[#733232] text-[#FFFFFF] font-bold text-xs py-3.5 rounded-xl transition-colors shadow-md"
                >
                  Submit Review
                </button>
              </form>
