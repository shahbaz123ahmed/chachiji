    ...
    <div className="bg-[#FFFFFF] min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-2">
        <div className="flex items-center gap-1.5 text-xs text-[#777777] font-medium">
          <NextLink href="/" className="hover:text-[#8B3E3E]">
            Home
          </NextLink>
          <ChevronRight className="w-3.5 h-3.5" />
          <NextLink href="/shop" className="hover:text-[#8B3E3E]">
            Shop
          </NextLink>
          <ChevronRight className="w-3.5 h-3.5" />
          <span className="text-[#8B3E3E] font-bold">{categoryData.name}</span>
        </div>
      </div>

      {/* Editorial Category Hero - Solid #8B3E3E with solid #F7A77A tag */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        <div className="relative rounded-3xl overflow-hidden bg-[#8B3E3E] text-[#FFFFFF] p-8 sm:p-12 lg:p-16 border-2 border-[#733232] shadow-xl">
          ...
          <div className="relative z-10 max-w-2xl">
            <span className="inline-block bg-[#F7A77A] text-[#333333] text-[10px] font-bold uppercase tracking-widest px-3.5 py-1 rounded-full mb-3 shadow-sm">
              {categoryData.hindiName}
            </span>
            <h1 className="font-serif text-3xl sm:text-5xl font-bold leading-tight mb-3">
              {categoryData.name}
            </h1>
            <p className="font-serif text-lg sm:text-xl text-[#FCE9D6] italic mb-4 font-bold">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-2">
              {categoryData.highlights.map((highlight, i) => (
                <div key={i} className="flex items-center gap-2 text-xs text-[#FCE9D6] font-bold">
                  <CheckCircle2 className="w-4 h-4 text-[#F7A77A] shrink-0" />
                  <span>{highlight}</span>
                </div>
              ))}
            </div>
        ...
        <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-20 border-t border-[rgba(51,51,51,0.10)]">
          ...
          <div className="space-y-4">
            {categoryFaqs.map((faq, idx) => (
              <div
                key={idx}
                className="bg-[#FCE9D6] p-6 rounded-2xl border border-[rgba(51,51,51,0.10)] shadow-xs"
              >
                <h4 className="font-serif text-base font-bold text-[#333333] mb-2 flex items-start gap-2">
                  <HelpCircle className="w-5 h-5 text-[#8B3E3E] shrink-0 mt-0.5" />
                  <span>{faq.question}</span>
                </h4>
                <p className="text-xs sm:text-sm text-[#333333] font-medium leading-relaxed pl-7">
                  {faq.answer}
                </p>
              </div>
            ))}
